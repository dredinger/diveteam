...
@section ('content')
	...
	@php
		$jpm_checked = ($log->jumppack_masks == 1) ? ' checked' : '';
		$dis_checked = ($log->disinfectants == 1) ? ' checked' : '';
		$ckl_checked = ($log->sd_checklist == 1) ? ' checked' : '';
		$ath_checked = ($log->aed_third == 1) ? ' checked' : '';
		$ase_checked = ($log->aed_second == 1) ? ' checked' : '';
	@endphp
	...
	<form action="{{ route('dso.logs.update', $log->id) }}" method="POST" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<input type="number" class="form-control" pattern="[0-9]*" name="psi_uts" id="psi_uts" required maxlength="4" placeholder="0" autocomplete="off" min="0" max="5000" step="100" value="{{ $log->psi_uts }}" />
		</div>
		<div class="form-group">
			<label for="psi_sw"><strong>Shipwreck PSI</strong></label>
			<input type="number" class="form-control" pattern="[0-9]*" name="psi_sw" id="psi_sw" required maxlength="4" placeholder="0" autocomplete="off" min="0" max="5000" step="100" value="{{ $log->psi_sw }}" />
		</div>
		<div class="form-group">
			<label for="psi_dr"><strong>Dining Room PSI</strong></label>
			<input type="number" class="form-control" pattern="[0-9]*" name="psi_dr" id="psi_dr" required maxlength="4" placeholder="0" autocomplete="off" min="0" max="5000" step="100" value="{{ $log->psi_dr }}" />
		</div>
		<div class="form-group">
			<label for="psi_bank"><strong>Bank System PSI</strong></label>
			<input type="number" class="form-control" pattern="[0-9]*" name="psi_bank" id="psi_bank" required maxlength="4" placeholder="0" autocomplete="off" min="0" max="5000" step="100" value="{{ $log->psi_bank }}" />
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<input type="number" class="form-control" pattern="[0-9]*" name="psi_oxy_third" id="psi_oxy_third" required maxlength="4" placeholder="0" autocomplete="off" min="0" max="5000" step="100" value="{{ $log->psi_oxy_third }}" />
		</div>
		<div class="form-group">
			<label for="psi_oxy_second"><strong>2nd Floor O<sub>2</sub> Kit PSI</strong></label>
			<input type="number" class="form-control" pattern="[0-9]*" name="psi_oxy_second" id="psi_oxy_second" required maxlength="4" placeholder="0" autocomplete="off" min="0" max="5000" step="100" value="{{ $log->psi_oxy_second }}" />
		</div>
		<div class="form-group">
